...
const pubRegSlice = createSlice({
  ...
  reducers: {
    ...
    updateSubGroup: (state, action) => {   
      state.subGroup = action.payload
    },
    selectSubGroup: (state, action) =>{
      let acitonArray = action.payload.split("_");
      state.subGroup = state.subGroup.map((d) =>
        d.main_id == acitonArray[1]
          ? {
              ...d,
              sub_names: d.sub_names?.reduce((a, b) => {
...
export const { initLoader, initPreLoader,initPostLoader, updateUserInfo , updateSubGroup, selectSubGroup, initSubGroup} = pubRegSlice.actions;
