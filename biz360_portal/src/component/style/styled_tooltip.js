...
export const TooltipTarget = styled.button`
  ...
  font-size: ${({ headertitlefont, theme }) => getBNFont(theme.fontSize[headertitlefont ? headertitlefont : 'font'])};
  font-weight: bold;
  cursor: inherit;
  display: flex;
  align-items: center;
...
//   left: -50%;
  ${({ position }) => {
        switch (position) {
          ...
          left:10%;
          width: ${({ tbwidth }) => tbwidth? tbwidth: "100%"};
          top: calc(100% + 5px);
        `;
            case "left":
                return css`
          margin-right: 0;
          width: ${({ tbwidth }) => tbwidth? tbwidth: "max-content"};
          left: unset;
          top: 50%;
          right: calc(100% + 5px); 
        `;
