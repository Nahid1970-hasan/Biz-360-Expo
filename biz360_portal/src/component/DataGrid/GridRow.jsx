      ...
      (k.hide) && !k.key ?
        ...
        k.type == 'checkbox' ? (
          <CellItem key={j} width={'30px'} >
            <Checkbox size="sm" slctcolor="inputFont" hvcolor={'girdHeader'} checked={row.checkbox} onClick={updateCheckedRow} />
          </CellItem>
        ) : k.type == 'input' ? (
              ...
              (
                ...
                (
                  ...
                  (
                    ...
                    (
                    ...
                    ) : k.child ? (<CellItem child={k.child} key={j} type={k.type} smhide={k.smhide?"true":""} hide={k.hide?"true":""} width={k.width} overflow={"hidden"} fontSize={"girdBodyFontSize"} alignment={k.alignment}>
                      <div ref={k.key && rowRef}>
                      ...
                      <CellItem key={j} fontFamily={k.fontFamily} type={k.type} smhide={k.smhide?"true":""} hide={k.hide?"true":""} width={k.width} overflow={"hidden"} fontSize={"girdBodyFontSize"} alignment={k.alignment}>
                        <div ref={k.key && rowRef}>
                          {row[k.field] == undefined ? "---" : row[k.field].length < 1 ? "---" : row[k.field]}
                          {k.child ? <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <CellItem type={k.type} hide={k.hide?"true":""} smhide={k.smhide?"true":""} width={k.width} overflow={"hidden"} fontSize={"girdBodyFontSize"} alignment={k.alignment}>
                              <div ref={k.key && rowRef}>
                                {row[k.child[0].field] == undefined ? "---" : row[k.child[0].field].length < 1 ? "---" : row[k.child[0].field]}
                              </div>
                            </CellItem>
                            <CellItem type={k.type} smhide={k.smhide?"true":""} hide={k.hide?"true":""} width={k.width} overflow={"hidden"} fontSize={"girdBodyFontSize"} alignment={k.alignment}>
                              <div ref={k.key && rowRef}>
                                {row[k.child[1].field] == undefined ? "---" : row[k.child[1].field].length < 1 ? "---" : row[k.child[1].field]}
                              </div>
                            </CellItem>
