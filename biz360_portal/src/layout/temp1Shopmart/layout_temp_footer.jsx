...
export const FooterMMenu = styled.div`
  width: 100%;
  display: flex;
  padding: 5px 0;
...
export const FooterTempLayout = () => {
    ...
    useEffect(() => {
        ...
        setActiveSocial(ftData?.social_media?.link?.filter((b) => +b.show == 1) || []);
                ...
                <Container border={"none"}>
                    <Flex row="row">
                        <Flex padding="10px 0 0 0 !important" md={3}>
                            ...
                            <SizeBox />
                            {
                                activeSupportLink?.map((d, i) => <div key={i}>
                                    {d.type == "mobile" ? <Typography fntsize="bodySubTitleFontSize" color="bodyContent" width={"100%"} txtalign="left">
                                        <FooterMenu><a href={"tel:" + d?.link}>{d?.link ? d?.link : d?.name ? d?.name : t("mobile" + (i + 1))}</a> </FooterMenu>
                                    </Typography> : d.type == "email" ?
                                        <Typography fntsize="bodySubTitleFontSize" color="bodyContent" width={"100%"} txtalign="left">
                                            <FooterMenu><a href={"mailto:" + d?.link} >{d?.link ? d?.link : d?.name ? d?.name : t("email" + (i + 1))}</a></FooterMenu>
                        ...
                        </Flex>
                        {
                            activeImpLink?.length > 0 ? <Flex padding="10px 0 0 0 !important" md={3}>
                                <Typography margin="0 0 0 10px" fntsize="bodyTitleFontSize" color="bodyContent" fntweight="bold" width={"100%"} txtalign="left">
                    ...
                    <Flex row="row" justifycenter="true">
                        <Flex md={12} padding="0!important">
                            <Typography fnfamily="var(--dashboard-font)" fntsize="bodyTitleFontSize" color="primaryFont" width={"100%"} margin="5px 0">
                                Copyright © {getCurrentYear()} {profilePubData?.basicData?.biz_name || ""} | All Rights Reserved to BIZ360Expo
                            </Typography>
                        </Flex>
                    </Flex>
                </Container>
