...
import { updateBizCp1 } from "./profile_slice";
...
export const ProfileCp_1Modal = ({ add,
    open,
    setOpen = () => { },
}) => {
    ...
    const data = user?.profile?.cp1_info;
    ...
    const validate = (Values) => {
        ...
        if (!Values.mobile) {
            errors.mobile = t("err_contact_no");
        } else if (!/(^(01){1}[3456789]{1}(\d){8})$/i.test(Values.mobile)) {
            errors.mobile = t("err_contact_no_invalid");
        }
