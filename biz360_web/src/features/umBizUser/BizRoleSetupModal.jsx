...
import { selectAllModule, selectModule, updateBIZMenuData,initLoader } from "./biz_menu_setup_slice";



export const BizRoleSetupModal = ({ open, setOpen, data }) => {

    const bizmenugroupdata = useSelector((state) => state.bizmenugroupdata);
    ...
    useEffect(() => {
        setSelectAll(bizmenugroupdata?.menuList?.every((d)=>d.sub_module?.every((b)=>b.selected)))
        ...
        <Modal md={8} sm={10} xs={10} open={open} setOpen={setOpen} onClose={() => { setOpen(false); }} title={t("assign_role", { lng: menuLang })} outsideclick>
            <CardHeaderButton>
                ...
                >
                    ...
                    {
                        bizmenugroupdata?.langlist?.map((d, i) => <option key={i} value={d.language_code}>{d.language_name}</option>)
                    }
                </Select>
                ...
                <PrimaryButton
                    ...
                    onClick={() => {
                         dispatch(updateBIZMenuData(data)
                        )
                         }}
                >
            ...
            <div>
                ...
                <Flex row="row">
                    {
                        bizmenugroupdata?.menuList?.map((module, i) => (<Flex key={i} padding={"5px !important"} md={4} xs={12}>
                            <InfoCard >
