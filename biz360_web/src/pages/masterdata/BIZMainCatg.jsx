...
export const BIZMainCatgPage = () => {
  ...
  useEffect(() => {
    if (bizMainGroupData?.list.length > 0) {
      const firstGroup = bizMainGroupData.list[0];
      set_group_id(firstGroup.group_id);
      set_group_name(firstGroup.group_name);
      dispatch(loadBIZMainCatgData({ group_id: firstGroup.group_id }));
    }
  }, [bizMainGroupData.list]);
            ...
            <Flex padding="0 !important" md={6} sm={12} xs={12}>
              <InlineFlex justifycontent="start">
                <Select
                  ...
                  name="group_id"
                  onChange={(e) => {
                    const grp = bizMainGroupData?.list?.find((d) => d.group_id == e.target.value);
                    set_group_name(grp.group_codename);
                ...
                >
                  ...
                  {bizMainGroupData?.list?.map((d, i) => (
                    <option key={i} value={d.group_id}>
                      {getValueByLang(d.group_codename || "{}", lang)}
                    </option>
                  ))}
                </Select>
            ...
            <Flex padding="0 !important" md={6} sm={12} xs={12}>
              <InlineFlex justifycontent="end">
                ...
                >
                  ...
                  {bizMainGroupData?.langlist?.map((d, i) => (
