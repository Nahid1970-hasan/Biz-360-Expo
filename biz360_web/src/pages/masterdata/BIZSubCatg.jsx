...
export const BIZSubCatgPage = () => {
  ...
  const [main_name, set_main_name] = useState("");
  ...
  useEffect(() => {
    if (bizMainCatgData?.list.length > 0 && bizSubCatgData?.list.length < 1 ) {
      const firstMainCatg = bizMainCatgData.list[0];
      set_main_id(firstMainCatg.main_id);
      set_main_name(firstMainCatg.main_name);
      dispatch(loadBIZSubCatgData({ main_id: firstMainCatg.main_id }));
    }
  }, [bizMainCatgData.list]);
            ...
            <Flex padding="0 !important" md={6} sm={12} xs={12}>
              <InlineFlex justifycontent="start">
                <Select
                  ...
                  name="main_id"
                  onChange={(e) => {
                    const mainCatg = bizMainCatgData?.list?.find((d) => d.main_id == e.target.value);
                    set_main_name(mainCatg.main_name);
                ...
                >
                  ...
                  {bizMainCatgData?.list?.map((d, i) => (
                    <option key={i} value={d.main_id}>
                      {getValueByLang(d.main_name || "{}", lang)}
                    </option>
                  ))}
                </Select>
            ...
            <Flex padding="0 !important" md={6} sm={12} xs={12}>
              <InlineFlex justifycontent="end">
                ...
                >
                  ...
                  {bizMainCatgData?.langlist?.map((d, i) => (
                    <option key={i} value={d.language_code}>
                      {d.language_name}
                    </option>
                  ))}
                </Select>
