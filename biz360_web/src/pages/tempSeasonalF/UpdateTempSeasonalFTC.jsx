...
export const UpdateTempSeasonalFTCPage = () => {
    ...
    useEffect(() => {
        dispatch(loadPage({
            title: 'terms_condition_template',
            button: false
        }))
    }, []);

    useEffect(() => {
        var infoBodyData = JSON.parse(admTempData?.tempInfoData?.template_body || "{}")?.page5 || {}; 
        setSectionPrvcData(infoBodyData);
    }, [admTempData]);
 
    const submitForm = (e) => { 
        var datak = { 'page_data': output };
        var data = { "template_id": admTempData.tempInfoData?.template_id || 0, "template_body": JSON.stringify({ 'page5': datak }) };
