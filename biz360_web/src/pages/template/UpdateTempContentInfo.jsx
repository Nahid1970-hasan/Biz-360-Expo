...
import { CardHeader, InfoCard, ShadowCard } from "../../component/style/styled_card";
import { InlineDiv } from "../../component/style/styled_inlineflex";
import { PrimaryButton } from "../../component/style/styled_button";
import { SetupTempNavModalPage } from "./SetupTempNavModal";
import { SetupTempFooterModalPage } from "./SetupTempFooterModal";
import * as feather from "feather-icons/dist/feather.min";

export const UpdateTempContentInfoPage = () => {
    ...
    useEffect(() => {
        ...
        set_module_data(admTempData?.menuList?.filter((d) => d.menu_position == "header") || []);
        setLangList(admTempData?.langList || []);
        setFooterData(infoData || {});
        setActiveImpLink(admTempData?.menuList?.filter((d) => d.menu_position == "footer") || []);
                                ...
                                
                                {
                                    activeImpLink?.map((d, i) =>
                                        <Typography key={i} fntsize="bodySubTitleFontSize" color="bodyContent" width={"100%"} txtalign="left">
                                            <FooterMenu><a ><label>{getValueByLang(d.menu_name || "{}") || "Link " + (i + 1)}</label></a></FooterMenu>
                                        </Typography>)
                                }
                            </Flex>
