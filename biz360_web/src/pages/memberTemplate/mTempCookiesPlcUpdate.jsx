...
export const MUpdateTempCookiesPlcPage = () => {
  ...
  const [daysLeft, setDaysLeft] = useState(TEMPCONST.dayleft);
    ...
    useEffect(() => {
        if (tempInfo.template_id > 0) {
            dispatch(loadMTempBodyData({ template_id: tempInfo.template_id, page_name: "cookies-policy", section_name: "", sub_section_name: "" }));
        }
    }, [tempInfo]);
    ...
    const handleOnSubmit = (e) => {
        dispatch(saveMTempBodyData({ template_id: tempInfo.template_id, page_name: "cookies-policy", photo_name: '', sub_section_name: "", section_name: "section1", 'content': JSON.stringify({ 'page_data': output }) }))
